...
//2.2 Them mot phan tu vao danh sach
int Search (int a[], int x, int &n)
{
	int i=0;
	while(!(x<a[i] && x>a[i-1]))
			i++;
	// da co i, vi tri them, doi cac fan tu sau i qua 1 vi tri
	for(int j= n; j>i; j--)
		a[j]=a[j-1];
	n++;
	a[i]=x;
...
int main()
{
	...
	int m=7;
	system("cls");
	cout<<"----------BAI TAP 2, CHUONG 2 : DANH SACH DAC----------"<<endl;
	...
	do
	{
		cout<<"Chon so de thuc hien: "<<endl;
		cin>> choice;
		switch (choice)
		{
		case 0:
		...
		case 5:
			...
			if( i == 1)
			{
				...
				cout<<"Khong co phan tu nao xoa !"<<endl;
			break;
		case 6:
			cout<<"\n Goodbye!"<<endl;
			break;
